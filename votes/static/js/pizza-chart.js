/* Colour picker function from http://stackoverflow.com/a/32257791 */
function hex (c) {
...
function convertToRGB (hex) {
  var colour = [];
  colour[0] = parseInt ((trim(hex)).substring (0, 2), 16);
  colour[1] = parseInt ((trim(hex)).substring (2, 4), 16);
  colour[2] = parseInt ((trim(hex)).substring (4, 6), 16);
  return colour;
}
...
/**
 * Taken from http://bootsnipp.com/snippets/84D3
 */
...
;(function ($, window, document, undefined) {
    ...
    var Pizza = {
        ...
        settings: {
            ...
            pie_scaling_factor: 0.5,    // between >0 and 1, 1 being full size, and >0 being tiny
        ...
        data: function (legend, options) {
            ...
            var colours = generateColor('#ff8080', '#990000', data.length);

            for (var i = 0; i < data.length; i++) {
                data[i].color = "#" + colours[i];
            }
        ...
        pie: function (legend) {
            ...
            var settings = legend.data('settings'),
                ...
                angles = [],
                start_angle = 0,
                base = ($(this.identifier(legend)).width() - 4) * settings.pie_scaling_factor;
            ...
            for (i = 0; i < data.length; i++) {
                ...
                path.node.setAttribute('data-cx', String(cx));
                path.node.setAttribute('data-cy', String(cy));
        ...
        svg: function (legend, settings) {
            var container = $(this.identifier(legend)),
                svg = $('svg', container),
                width = container.width() * settings.pie_scaling_factor,
                height = container.width() * settings.pie_scaling_factor;
            ...
            svg.node.setAttribute('width', String(width + settings.percent_offset));
            svg.node.setAttribute('height', String(height + settings.percent_offset));
